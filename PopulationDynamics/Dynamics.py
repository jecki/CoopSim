...
from Compatibility import array, asarray, diagonal, dot, HAS_NUMERIC, uniform
...
def GenTuples(dimension, count):
    ...
    if dimension == 1:
        ...
        raise ValueError("tuple dimension " + dimension + " is not allowed!")
...
def RandomSelect(distribution):
    ...
    def rselect(slices, r, a, b):
        ...
        m = (a + b) // 2
        if r >= slices[m]:
            return rselect(slices, r, m, b)
        else:
            return rselect(slices, r, a, m)

    if len(distribution) == 0:
        raise ValueError("Cannot select an element from an empty distribution")
    slices = [0.0]
    for p in distribution:
        slices.append(p + slices[-1])
...
def RandomSample(distribution, n):
    ...
    # assert n <= len(distribution), "Cannot select %i individuals from a
    # population of less than %i individuals"%(n,n)
    if n > len(distribution):
        n = len(distribution)
...
def _numeric_QuickFitness2(population, payoff):
    ...
    A faster replacement for 'Fitness2' if correlation e = 0.0.
    """
    return dot(payoff, asarray(population))


if HAS_NUMERIC:
    QuickFitness2 = _numeric_QuickFitness2
else:
    QuickFitness2 = _QuickFitness2
...
def _FunctionSelector(population, N, samples, stdFunction, sampleFunction,
    ...
    combinations = L**N
    if samples < 1.0:
        samples = int(combinations * samples + 0.5)
    if samples == 0:
        samples = 1
    if samples < combinations:
        return sampleFunction(population, *(baseParams + (samples,)))
    else:
        return stdFunction(population, * baseParams)
...
class GenFitnessFunction(object):
    def __init__(self, payoff, e=0.0, N=2, samples=1.0):
        ...
        if samples <= 0:
            raise ValueError("samples %f <= 0 !" % samples)

        if N == 1:
            if isinstance(e, 0.0.__class__) and (e != 0.0):
                warnings.warn("Correlation e != %f is meaningless if N == 1"
                              % e, UserWarning)
            self.stdFunction = lambda p: array([payoff[i, ]
            ...
            raise ValueError("N must be >= 1, not %i !" % N)
...
def _Replicator(population, fitness, noise):
    ...
    for i in range(L):
        n[i] *= f[i] - random.uniform(0, f[i] * noise)
    N = sum(n)
    if N == 0.0:  # <- this should never be true, except for precision errors!
        n = list(population)
        for i in range(L):
            n[i] *= 1.0 - random.uniform(0, noise)
        N = sum(n)
        if N == 0.0:
            return population
...
if HAS_NUMERIC:
    Replicator = _numeric_Replicator
else:
...
if HAS_NUMERIC:
    QuickReplicator = _numeric_QuickReplicator
else:
    QuickReplicator = _QuickReplicator
