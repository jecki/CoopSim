...
#def QuickTest(strategy):
#    """-> any exception caused by "strategy" when running the tests,
#    otherwise "None" is returned."""
#    def Test(strategy, opMoves):
#         try:
#             sm = [strategy.firstMove()]
#             if sm[0] not in [0, 1]: return ValueError()
#             for i in xrange(1, 1001):
#                 sm.append(strategy.nextMove(sm, om[:i]))
#                 if sm[-1] not in [0, 1]: return ValueError()
#        except
...
class RandomizingStrategy(Strategy):
    """A root class for randomizing strategies. This keeps implementors
    of random strategy classes from having to define a constructor just
    to set the randomizing flag. Just derive your random strategy from
    class RandomizingStratgey instead of class Strategy and everything
    will be fine!
    """
    def __init__(self):
...
class SmarterTatForTit(Strategy):
    """Plays 'tat for tit'. Punishes defections like TFT, but also
    starts with a defection. Excuses a defection in the first round,
    but only then."""
    def firstMove(self):
        return 0
    def nextMove(self, myMoves, opMoves):
        if len(opMoves) == 1 and opMoves[0] == 0: return 1
        else: return opMoves[-1]
smartertatfortit = SmarterTatForTit()
...
class Random(RandomizingStrategy):
    """Plays random moves."""
    def firstMove(self):
...
signalingCheater = SignalingCheater()
...
#        else: return opMoves[-1]
class GenerousTFT(Strategy):
    """Plays 'tit for tat', but forgoes every 5th punishment."""
    def firstMove(self):
        self.punishmentsCounter = 5
        return 1
    def nextMove(self, myMoves, opMoves):
        if opMoves[-1] == 1:
            return 1
        else:
            self.punishmentsCounter -= 1
            if self.punishmentsCounter == 0:    # be gracious
                self.punishmentsCounter = 5
                return 1
            else:
                return 0
generousTFT = GenerousTFT()


class Joss(RandomizingStrategy):
...
class Downing(Strategy):
    ...
    tries to deceive. (Note: This strategy is different than
    the strategy DOWNING described in Axelrod's "Evolution of
    Cooperation". At best it is a very much simplified version
    of that strategy!)
    """
    def __repr__(self):
...
class Tranquilizer(RandomizingStrategy):
...
class ParameterizedTFT(RandomizingStrategy):
    ...
    def __init__(self, goodrate=0.2, evilrate=0.05):
        ...
        RandomizingStrategy.__init__(self)
...
parameterizedTFT = ParameterizedTFT()
...
class TwoStateAutomaton(Strategy):
    ...
    """
    def __repr__(self):
        return self.__class__.__name__+"("+repr(self.progString)+")"
...
stefanPennartz = Stefan_Pennartz()
...
aquaintanceAndTrust = AcquaintanceAndTrust()


class Envy(Strategy):
    """submitted by Bernd Lahno
    1. starts with cooperation
    2. cooperates as long as the opponent has not defected
    more often in the previous rounds than envy.
    """
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if sum(opMoves) < sum(myMoves):  return 0
        else: return 1
envy = Envy()


class ContriteTFT(Strategy):
    """Described in Jianzhong Wu / Robert Axelrod:
    How to Cope with Noise in the Iterated Prisoner's Dilemma
    in: Journal of Conflict Resolution, Vol. 39, No. 1, March 1995
    """
    def firstMove(self):
        self.state = "content"
        return 1
...
#
# The following strategies have hardly been tested and may not (yet) work
# as intended. (Therefore, I have commented them out for the time being!)
#
###############################################################################
...
class SoftGrim(Strategy):
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if sum(opMoves) < len(opMoves)-3:
            return 0
        else:
            return 1

class LevelPunisher(Strategy):
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if len(opMoves) <= 10: return 1
        else:
            if float(len(opMoves) - sum(opMoves)) / len(opMoves) > 0.2:
                return 0
            else: return 1

class TrickyLevelPunisher(Strategy):
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if len(opMoves) <= 10: return 1
        ...
        elif len(opMoves) <= 100:
            if float(len(opMoves) - sum(opMoves)) / len(opMoves) > 0.1:
                return 0
            else: return 1
        else:
            if float(len(opMoves) - sum(opMoves)) / len(opMoves) > 0.05:
                return 0
            else: return 1

class Resurrection(Strategy):
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if len(myMoves) >= 5 and myMoves[-5:] == [0,0,0,0,0]:
            return 1
        else:
            return opMoves[-1]

class DoubleResurrection(Strategy):
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if len(myMoves) >= 5 and myMoves[-5:] == [1,1,1,1,1]:
                return 0
        elif len(myMoves) >= 5 and myMoves[-5:] == [0,0,0,0,0]:
                return 1
        else:
            return opMoves[-1]

class TwoTitsForTwoTats(RandomizingStrategy):
    def firstMove(self):
        return 1
    def nextMove(self, myMoves, opMoves):
        if random.random() <= 0.1:
            ...
            if opMoves[-2:] == [0, 0]:
                return 0
            elif len(opMoves) >= 3 and opMoves[-3:-1] == [0, 0]:
                return 0
            else: return 1
        else: return 1

class FingerPrint(Strategy):
    def firstMove(self):
        self.switch = ""
        return 1
    def nextMove(self, myMoves, opMoves):
        if len(myMoves) <= 4:
            return [1, 0, 1, 0, 1][len(myMoves)]
        elif len(myMoves) == 5:
            if opMoves[0:5] == [1,1,1,1,1]:
                self.switch = "B"
            if opMoves[0:5] == [1,0,1,0,1] or opMoves[0:5] == [1,1,0,1,0] or \
               opMoves == [0,1,0,1,0]:
                self.switch = "A"
            else: self.switch = "C"
            if self.switch == "A": return 1
            else: return 0
        else:
            if self.switch == "A":
                return opMoves[-1]
            elif self.switch == "B":
                if len(opMoves) <= 8:  return 0
                elif len(opMoves) == 9:
                    if opMoves[-1] == 0:
                        self.switch = "D"
                        return 1
...
# of generations until the system converges to the "Always
# Defect"-equilibirum!
