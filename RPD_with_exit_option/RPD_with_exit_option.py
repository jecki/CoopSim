...
"""Simulation of the reiterated Prisoner's Dilemma with exit option.

The idea to this comes from Rudolf Schüßler. For a discussion of the 
reiterated PD with exit option see:
Rudolf Schüßler: Kooperation unter Egoisten: Vier Dilemmata,
R. Oldenbourg, Muenchen 1997, S.61ff. The concepts and strategies
in this program are based on Schuessler's book."""
...
def OneGeneration(distribution, rounds):
    ...
    the population shares of QUIT FOR TAT and HAWK player's. 'rounds'
...
def Schuessler():
    ...
    graph.addPen("QUIT FOR TAT", Gfx.Pen(color = Gfx.GREEN, lineWidth = Gfx.MEDIUM))
    graph.addPen("HAWK", Gfx.Pen(color = Gfx.RED, lineWidth = Gfx.MEDIUM))

    # Calculate the population dynamics and plot the graph.

    population = initial_distribution
    graph.addValue("QUIT FOR TAT", 0, population[0])
    graph.addValue("HAWK", 0, population[1])
    fitness = lambda p: OneGeneration(p, rounds)
    for g in range(1, generations+1):
        population = PopulationDynamics(population, fitness)
        graph.addValue("QUIT FOR TAT", g, population[0])
        graph.addValue("HAWK", g, population[1])         
        if g % (generations/10) == 0:  gfx.refresh()

    # To save the graphics in eps uncomment the following line
    graph.dumpPostscript("Schuessler_RPD_1.eps")
